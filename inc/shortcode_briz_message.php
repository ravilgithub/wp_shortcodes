...
/**
 * Класс реализует шорткод "briz_message",
 ...
 * @author Ravil
 */
class Briz_Message_Shortcode extends Briz_Shortcodes {
	public $name   = 'briz_message';
	public $assets = [
		'css' => [
			'briz_message' => [
				'deps' => [
	 ...
	 *
	 * Функция обработчик шорткода "briz_message".
	 *
	...
	public function shortcode_briz_message( $atts, $content, $tag ) {
		...
		if ( ! empty( $atts[ 'border_width' ] ) ) {
			$this->add_inline_styles( '.briz-woocommerce-info-inner', 'border-width', $atts[ 'border_width' ] );
			if ( ! empty( $atts[ 'border' ] ) ) {
				$this->add_inline_styles( '.briz-woocommerce-info-inner', 'border-color', $atts[ 'border' ] );
			}

			if ( ! empty( $atts[ 'border_top' ] ) ) {
				$this->add_inline_styles( '.briz-woocommerce-info-inner', 'border-top-color', $atts[ 'border_top' ] );
			}

			if ( ! empty( $atts[ 'border_top_width' ] ) ) {
				$this->add_inline_styles( '.briz-woocommerce-info-inner', 'border-top-width', $atts[ 'border_top_width' ] );
			}
		}
		...
		if ( ! empty( $atts[ 'back' ] ) ) {
			$this->add_inline_styles( '.briz-woocommerce-info-inner', 'background-color', $atts[ 'back' ] );
		}

		if ( ! empty( $atts[ 'text_color' ] ) ) {
			$this->add_inline_styles( '.briz-woocommerce-info-inner', 'color', $atts[ 'text_color' ] );
		}

		if ( ! empty( $atts[ 'text_size' ] ) ) {
			$this->add_inline_styles( '.briz-woocommerce-info-inner', 'font-size', $atts[ 'text_size' ] );
		}
...
?>
		<div id="<?php echo $id ?>" class="<?php echo $atts[ 'class' ]?>" role="alert">
			<span class="briz-woocommerce-info-inner">
