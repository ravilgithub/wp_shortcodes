...
/**
 * Класс реализует шорткод "briz_image",
 ...
 *  position="left"
 *  target="_self"
 *  onclick='console.log("briz_image onclick"); return false'
 * ]
...
class Briz_Image_Shortcode extends Shortcodes {
	...
	public $default_atts  = [
		...
		'width'        => '',
		'border'       => '#eee',
		'border_width' => '1px', // 0px что бы убрать рамки, но оставить минимальные отступы в .shortcode_briz_image_label
		'text_color'   => '#90c948',
		'text_size'    => '1em',
		'position'     => '',
		'target'       => '',
	 ...
	 *
	 * Функция обработчик шорткода "briz_image".
	 *
	...
	public function shortcode_briz_image( $atts, $content, $tag ) {
		...
		if ( ! empty( $atts[ 'width' ] ) ) {
			// .shortcode_briz_image_inner_wrapper { width }
			$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper', 'width', $atts[ 'width' ] );
		}
		...
		if ( ! empty( $atts[ 'border_width' ] ) ) {
			...
			if ( preg_match( $exp, $atts[ 'border_width' ], $matches ) ) {
				...
				// .shortcode_briz_image_inner_wrapper .shortcode_briz_image_label { padding-top }
				$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper .shortcode_briz_image_label', 'padding-top', $label_top_padding );

				// .shortcode_briz_image_inner_wrapper .shortcode_briz_image_label { padding-bottom }
				$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper .shortcode_briz_image_label', 'padding-bottom', $label_bottom_padding );
			}

			if ( 1 === $type ) {
				// .shortcode_briz_image_inner_wrapper { border-width }
				$this->add_inline_styles( "&.{$default_class}_border .shortcode_briz_image_inner_wrapper", 'border-width', $atts[ 'border_width' ] );
			}

			if ( 2 === $type ) {
				// .shortcode_briz_image_inner_wrapper { padding }
				$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper', 'padding', $atts[ 'border_width' ] );
			}

			if ( ! empty( $atts[ 'border' ] ) ) {
				// .shortcode_briz_image_inner_wrapper { border-color }
				$this->add_inline_styles( "&.{$default_class}_border .shortcode_briz_image_inner_wrapper", 'border-color', $atts[ 'border' ] );
			}
		}

		if ( ! empty( $atts[ 'border' ] ) ) {
			if ( 1 === $type ) {
				// .shortcode_briz_image_inner_wrapper .shortcode_briz_image_label { background-color }
				$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper .shortcode_briz_image_label', 'background-color', $atts[ 'border' ] );
			}
		}

		if ( ! empty( $atts[ 'text_color' ] ) ) {
			// .shortcode_briz_image_inner_wrapper:hover .shortcode_briz_image_label { color }
			$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper:hover .shortcode_briz_image_label', 'color', $atts[ 'text_color' ] );
		}

		if ( ! empty( $atts[ 'text_size' ] ) ) {
			// .shortcode_briz_image_inner_wrapper .shortcode_briz_image_label:hover { font-size }
			$this->add_inline_styles( '.shortcode_briz_image_inner_wrapper .shortcode_briz_image_label:hover', 'font-size', $atts[ 'text_size' ] );
		}
...
?>
		<figure  id="<?php echo $id ?>" class="<?php echo $atts[ 'class' ] ?>">
			<span class="shortcode_briz_image_inner_wrapper">
				<span class="shortcode_briz_image_image_box">

					<?php if ( wp_http_validate_url( $atts[ 'img_url' ] ) ) : ?>
						<img src="<?php echo esc_url( $atts[ 'img_url' ] ) ?>" alt="ALT" />
					<?php endif; ?>

					<span class="shortcode_briz_image_image_mask"></span>

					<span class="shortcode_briz_image_icons_box">

						<?php if ( wp_http_validate_url( $atts[ 'img_url' ] ) ) : ?>
							<a href="<?php echo esc_url( $atts[ 'img_url' ] ) ?>" class="shortcode_briz_image_mfp_popup_icon">
								<i class="fa fa-search" aria-hidden="true"></i>
							</a>
						<?php endif; ?>

						<?php if ( $atts[ 'link' ] ) : ?>
							<a href="<?php echo $atts[ 'link' ] ?>" class="shortcode_briz_image_link_icon" <?php echo $rel, $target, $onclick ?>>
