...
class Bri_Blockquote_Shortcode extends Bri_Shortcodes {
	 ...
	 *
	 * Функция обработчик шорткода "bri_blockquote".
	 *
	...
	public function shortcode_bri_blockquote( $atts, $content, $tag ) {
		...
		if ( ! empty( $atts[ 'text_color' ] ) ) {
			$this->add_inline_styles( '.shortcode_bri_blockquote_summary', 'color', $atts[ 'text_color' ] );
		}
		...
		if ( ! empty( $atts[ 'decor_icon_color' ] ) ) {
			$this->add_inline_styles( '.shortcode_bri_blockquote_decor_icon .fa', 'color', $atts[ 'decor_icon_color' ] );
		}

		if ( ! empty( $atts[ 'author_icon_color' ] ) ) {
			$this->add_inline_styles( '.shortcode_bri_blockquote_author_icon .fa', 'color', $atts[ 'author_icon_color' ] );
		}

		if ( ! empty( $atts[ 'author_name_color' ] ) ) {
			$this->add_inline_styles( '.shortcode_bri_blockquote_author_name', 'color', $atts[ 'author_name_color' ] );
		}
...
?>
		<q id="<?php echo $id ?>" class="<?php echo $atts[ 'class' ] ?>">
			...
			<span class="shortcode_bri_blockquote_summary">
				...
				<?php if ( ! empty( $atts[ 'author_name' ] ) ) : ?>
					<span class="shortcode_bri_blockquote_author_info">
						...
						<span class="shortcode_bri_blockquote_author_details">
							<strong class="shortcode_bri_blockquote_author_name">

								<?php if ( ! empty( $atts[ 'author_link' ] ) && wp_http_validate_url( $atts[ 'author_link' ] ) ) : ?>
									<a href="<?php echo esc_url( $atts[ 'author_link' ] ) ?>" target="_blank" class="shortcode_bri_blockquote_author_link"><?php _e( $atts[ 'author_name' ], $lang_domain ) ?></a>
								<?php else : ?>
