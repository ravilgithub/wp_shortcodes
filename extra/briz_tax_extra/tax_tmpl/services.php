...
?>
		<section
			...
			class="showcase section services-page
	...
	public function get_content( $posts ) {
		...
		foreach ( $posts[ 'data' ] as $data ) :
			...
			if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post();
				...
				$post_excerpt = __( get_the_excerpt(), $this->lang_domain );
				$post_link = esc_url( get_permalink() );
				$post_more_link_text = __( 'Readmore...', $this->lang_domain );
?>
				<div class="col-xs-10 col-xs-offset-1 col-sm-4 col-sm-offset-0 services-item-wrap">
					<div class="icon-wrap">
<?php
					if ( array_key_exists( 'icon', $opts ) && $icon_name = esc_attr( $opts[ 'icon' ] ) ) :
?>
						<i
							class="fa fa-<?php echo $icon_name; ?>"
							aria-hidden="true"
						></i>
<?php
					endif;
